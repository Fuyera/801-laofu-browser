...
try:
    for name, expected, _ in GROUPS:
        ...
        if hashlib.sha256(text.encode()).hexdigest() != expected:
            raise RuntimeError('Patch digest mismatch: ' + name)
    ...
    for name, digest, message in GROUPS:
        patch = str(ROOT / (name + '.patch'))
        run(['git', 'apply', '--check', patch], name + '-patch-check')
        run(['git', 'apply', patch], name + '-patch-apply')
        run(['git', 'diff', '--check'], name + '-whitespace')
