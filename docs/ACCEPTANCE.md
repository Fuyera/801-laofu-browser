...
> 2026-09-15 对抗复核确认的 28 条缺陷已修复并验收，范围和分阶段证据见 [ADVERSARIAL_FIXES.md](ADVERSARIAL_FIXES.md)。最终 50/50 单元／接口、19/19 浏览器、8/8 双隔离及 2/2 接手通过；24 个矩阵项目在保留环境失败记录并补跑后均通过。以下为修复前 P4 历史验收，不代表日常环境已升级或最终包已冻结。
...
1. 日常 Chrome：已完成扩展安装、品牌、连接与 X 基础读取实测；该运行环境尚未升级最新修复。最新候选的停止／重连及保留浏览器验收仍待完成。X 列表遗漏经用户确认作为预览版已知限制暂缓处理；详见 [X 验收](X_READ_ACCEPTANCE.md)。
