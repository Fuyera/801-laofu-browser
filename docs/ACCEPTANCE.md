...
更新：2026-09-14（本地）。版本 `0.1.0-dev.2`，分支 `fuyera/p4-macos`。**完整矩阵 22/22 通过；P4 尚余日常 Chrome 人工入口及固定制品复验。** P5、跨平台、日本部署和跨端自动选择暂停。旧 dev.1 仅归档，历史包迁移已退出本轮门槛。
...
1. 日常 Chrome：待允许临时加载 laofu-browser 扩展后，只读取新建公开页，验证停止 worker 保留浏览器，再移除本次扩展。独立测试 Chromium 的 attach 通过不能代替该入口。
2. 提交源码文档，构建干净来源的固定 Mac 包/镜像；校验最终文件、独立安装与版本/提交一致性，再更新 `releases/DELIVERY.json`。旧档案不覆盖。
...
|P4|自动回归和实际 Codex 宿主通过；人工浏览器入口及固定交付待收口|
