...
当前源码 UI 已支持中英文。完整构建与前端类型检查通过；独立本机服务及真实 Chromium 验证了初始语言、选择记忆、登录错误、五区界面、输入与任务选择保留、任务状态及日期、凭据创建、经验保存、取消确认框，以及中英文移动布局。任务状态展示使用明确的合成持久任务，不计作新的网站执行证据。原有图文／控制台回归 7 组全部通过，使用真实 worker／扩展与本机合成页面，覆盖采集、ZIP 下载和安全预览。上游 58 文件与 23 工具基线仍保持不变。

见[本轮专项证据](evidence/ui-language.json)。构建后运行 `node scripts/ui-language-regression.mjs` 可复现语言检查。本轮未重跑完整交付矩阵、远程接手现场，也未升级日常服务或重生成固定发行包；已发布 dev.3 包尚不包含此次 UI 更新。
