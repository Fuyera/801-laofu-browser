...
**简体中文** | [English](USER_GUIDE.en.md)

laofu-browser 帮你在授权浏览器中读取网页、执行操作，并保存可查看、可下载的任务成果。当前代码为 `0.1.0-dev.2` Mac 开发候选版，已验证的是本人专用浏览器与指定测试场景。受限产品接入、日常 Chrome 安装和跨系统支持的当前状态见 [验收报告](ACCEPTANCE.md)。

源码版控制台现在支持中文与英文：登录页或侧栏点击 **简体中文 / English** 即可切换。首次按浏览器首选语言显示，之后在同一浏览器和服务地址记住选择。切换保留当前功能区、任务选择和未提交输入；日期、界面及错误摘要同步切换。文章、名称、笔记、API 标识和可展开的原始诊断保持原文。固定 dev.3 安装包尚未包含此次 UI 更新，使用当前源码构建可获得此功能。
...
|图文产物|下载文件或预览生成的安全 HTML；删除后新下载被拒绝，在途服务端下载中止|
...
- **文章打不开：**在任务使用的同一个浏览器查看登录、权限或站点提示；默认 `doctor` 不访问网站；加 `--site URL --profile 实际ID` 可进行有界浏览器检查。
- **图片不显示：**下载完整 ZIP 并保留相对目录；再检查 manifest 是否记录了下载失败或未支持的媒体。
- **配额不足：**默认全局产物额度 10 GiB、每产品 2 GiB；排队上限 20、未结束任务上限 24，可在产品凭据中调整，不自动删除旧成果。先保存需要的文件，再由本人选择删除。
- **程序恢复后任务仍挂起：**这是保留未知效果的保护状态；维护者需确认旧动作停止及外部结果，再恢复 profile。
- **Windows 或日本服务器：**跨平台和日本部署已暂停，准备脚本不代表通过验收。

## dev.2 的使用变化

- 采集会在有界时间内等待正文稳定并准备懒加载；没有稳定时返回部分完成和缺项。完成交付后只回收本任务新建的采集页；原有页面、等待人工和失败待核验的页面保留。默认总页预算为 8，达到上限时先处理保留页面。
- 页面提示频繁操作后，相同站点与出口进入持久冷却。可信 `Retry-After` 到期前不能创建新访问；恢复时间未知时，在“诊断与经验”核对后解除。解除不会重新执行旧任务。
- 接手连接票据有效期最多 60 秒、只能使用一次；断线后关闭接手窗口并重新打开即可领取新票据。
- 需要指定账号的浏览器由维护者配置站点与核验规则；错配或无法核验时等待人工，不把浏览器名称当成账号证明。公开网页默认不要求账号核验。
- 删除产物后，任务保留其 ID 和删除状态。已经下载到客户端的字节不能追回。未知或与声明不符的文件类型按普通二进制文件下载。

旧 dev.1 包仅归档；本轮使用新 dev.2 包独立安装，不承诺历史包迁移。P4 及最终包是否已冻结，以当前验收报告和 `releases/DELIVERY.json` 为准。
