...
更新：2026-09-14。用户已批准独立801产品P0→P5；当前交付0.1.0-dev.1本机开发版，**尚未通过P4，未完成P5/v1.0**。101仅保留交接入口，未改Fuyera/祈道/000业务代码，未公开发布，未动日本原huashu试验。
...
- 19项本地行为/HTTP测试、构建/契约通过；服务/桥在实际POST后回执前中断、旧worker重连、同键重投和act取消测试无重复提交。
...
- Mac固定Node/Chromium安装布局实际启动采集，完整性/额外文件拒绝、兼容升级、回退、启动失败恢复、备份、plist语法通过；未启用自启动。

完整要求映射、固定证据和限制见[验收报告](docs/ACCEPTANCE.md)、[兼容差异](docs/COMPATIBILITY.md)、[运行手册](docs/OPERATIONS.md)。最终本地制品位于releases，原始实测现场位于workspace，二者不包含关系：发行包不带账号/profile数据库。
...
1. P0隔离容器8项中7项通过；系统DNS把公网域名解析为198.18/15保留地址，公网正向探针失败，受限产品入口拒绝。Cloudflare DoH动作被自动审批拒绝，原因是目标域名元数据发送到该外部服务未经用户明确授权；未启用或绕过。等待用户答复/授权的网络环境修复后复验全部8项。
