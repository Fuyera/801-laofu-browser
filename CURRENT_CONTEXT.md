...
更新：2026-09-16。**0.1.0-dev.3，macOS Apple Silicon 开发者预览版**。产品定位为连接 AI 助手与浏览器的本地浏览器助手。19 份项目自有 Markdown 文档现均提供完整中英文版本、页首语言切换及同语言指南链接，功能介绍先展示任务与管理，再展示 23 种浏览器操作；版权与来源统一置于文末。网页控制台仍为中文。P5、跨平台和日本服务器部署继续暂停；X 列表遗漏作为已知限制暂缓处理。
...
包内文档为构建时快照，main 后续提交补充最终验收及全套双语文档，不改变固定程序。旧 dev.1／dev.2 候选仅本地归档，不覆盖同版本制品，不要求历史降级迁移。此次不提供 Docker 镜像发行包，隔离实现和既有证据保留于源码。后续打包包含根目录英文文档和完整双语 docs 目录。

## 验证

- 双语文档：19 对／38 份，401 处本地链接及锚点、18 组命令示例对应检查通过；Mac 文档复制 38/38 字节一致，Docker 文档输入路径及脚本语法检查通过。本次仅修改文档与打包文档清单，未重跑完整程序／浏览器回归，也未重生成固定发行包。
